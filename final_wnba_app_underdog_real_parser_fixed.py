...
def _is_wnba_context(*objs):
    """Prefer WNBA rows but avoid over-filtering Underdog's sparse objects.

    If the row explicitly says MLB/NFL/NHL/etc., reject it. If it says WNBA/women's, accept it.
    If it only has generic basketball text or numeric sport ids, allow it and let player/prop
    validation + diagnostics handle the rest. This is the key balance that makes props show
    without forcing fake lines.
    """
    blob = " ".join(_obj_text(o) for o in objs if isinstance(o, dict))
    if any(x in blob for x in ["wnba", "women", "women's", "womens"]):
        return True
    if any(x in blob for x in ["mlb", "baseball", "nhl", "nfl", "soccer", "tennis", "ufc", "mma"]):
        return False
    # Do not reject just because the string contains "nba"; WNBA contains NBA, and
    # Underdog sometimes only labels the sport as basketball.
    return True
