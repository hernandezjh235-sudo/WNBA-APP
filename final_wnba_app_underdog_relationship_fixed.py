...
def guess_underdog_stat_text(obj):
    ...
    keys = [
        ...
        "display_stat", "displayStat", "appearance_stat", "over_under_type",
        "overUnderType", "market", "market_name", "title", "name", "label"
    ]
    texts = []
    for k in keys:
        v = obj.get(k)
...
def guess_underdog_line(obj):
    if not isinstance(obj, dict):
        return None
    keys = ["stat_value", "statValue", "line", "value", "points", "point", "over_under_value", "overUnderValue"]
    for k in keys:
        v = obj.get(k)
        x = safe_float(v)
        if x is not None:
...
def _lookup_relation(obj, names):
    """Pull Underdog relationship ids from several possible JSON shapes."""
    if not isinstance(obj, dict):
        return None
    for name in names:
        for k in [name, f"{name}_id", f"{name}Id"]:
            v = obj.get(k)
            if isinstance(v, dict):
    ...
    if isinstance(rel, dict):
        for name in names:
            node = rel.get(name) or rel.get(name.replace("_", "-"))
...
def _is_wnba_context(*objs):
    """Prefer WNBA context, but do not require every line object to carry league text."""
    blob = " ".join(_obj_text(o) for o in objs if isinstance(o, dict))
    if any(x in blob for x in ["wnba", "women", "women's", "womens"]):
        return True
    # Underdog sometimes uses numeric sport ids and omits WNBA text from the line object.
    # If we have a basketball prop + WNBA app context, allow it unless it explicitly says NBA/men.
    if "nba" in blob and "wnba" not in blob:
        return False
    return "basketball" in blob or not any(x in blob for x in ["mlb", "baseball", "nhl", "nfl", "soccer", "tennis"])
...
def get_underdog_wnba_prop_rows():
    ...
    for url in UNDERDOG_URLS:
        ...
        if not data:
            log_request("Underdog WNBA", "FAILED", url)
            continue
...
def build_prop_board(games, selected_props, bankroll, default_odds, max_players_per_game=26):
    ...
    diagnostics = [{
        ...
        "Details": "Direct Underdog WNBA parser active. These rows are merged with The Odds API props.",
    }]

    for game in games:
        ev = match_event(game, events) or match_event(game, market_events)
        if not ev:
            diagnostics.append({
                "Game": f"{game.get('away')} @ {game.get('home')}",
                "Stage": "event_match",
                "Status": "NO ODDS EVENT MATCH",
                "Details": "Using direct Underdog fallback if rows are available.",
            })
            ev = {"id": None}

        prop_rows = get_event_prop_odds(ev.get("id"), ",".join(all_market_keys))
        # Merge direct Underdog props when The Odds API is thin. Underdog rows are real lines,
        # but may not include a reliable event id, so they are used as fallback support.
        if underdog_rows_all:
            existing_keys = {(normalize_name(r.get("Player")), r.get("Prop"), safe_float(r.get("Line"))) for r in prop_rows}
            for ur in underdog_rows_all:
                if ur.get("Prop") in selected_props:
                    key = (normalize_name(ur.get("Player")), ur.get("Prop"), safe_float(ur.get("Line")))
        ...
        diagnostics.append({
            ...
            "Details": f"Event {ev.get('id')} | markets {','.join(all_market_keys)} | Underdog merged {len(underdog_rows_all)}",
        })
        if not prop_rows:
            continue
