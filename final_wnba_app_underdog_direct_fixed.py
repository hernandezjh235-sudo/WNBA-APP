...
def extract_underdog_wnba_prop_rows(data):
    """Extract WNBA prop rows from Underdog without guessing lines.

    This is intentionally permissive on JSON shape but strict on prop/stat + line ranges.
    It only returns rows that map to one of our supported WNBA prop types.
    """
    rows = []
    ...
    for obj in objects:
        ...
        # Keep it WNBA/basketball-ish. Some Underdog responses mix sports.
        if not any(x in blob for x in ["wnba", "basketball", "women"]):
            continue
        stat_text = guess_underdog_stat_text(obj)
        prop = underdog_prop_code_from_text(stat_text + " " + blob[:500])
        if not prop:
            continue
        ...
        if not player or len(player.split()) < 2:
            # Try to find a name-looking value anywhere in the object.
            for v in obj.values():
                if isinstance(v, str) and len(v.split()) >= 2 and not underdog_prop_code_from_text(v):
                    if not any(bad in normalize_name(v) for bad in ["over", "under", "points", "rebounds", "assists", "fantasy"]):
                        player = v.strip()
                        break
        if not player or len(player.split()) < 2:
            continue
        ...
        rows.append({
            ...
            "Validation": "VALID_UNDERDOG_REAL_PROP_LINE",
        })
    return rows
