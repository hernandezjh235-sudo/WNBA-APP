...
# -------------------- NO-KEY WNBA SCHEDULE FALLBACK --------------------
@st.cache_data(ttl=300, show_spinner=False)
def fetch_wnba_schedule_fallback() -> Tuple[pd.DataFrame, str]:
    """Free no-key WNBA schedule fallback so the app never opens blank.
    This is NOT sportsbook odds. It only powers the Games tab when ODDS_API_KEY is missing.
    """
    rows: List[Dict[str, Any]] = []
    statuses: List[str] = []
    base = "https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/scoreboard"
    today = datetime.now(timezone.utc).date()
    # Check yesterday through next 14 days so off-days still show upcoming games.
    for offset in range(-1, 15):
        d = today + timedelta(days=offset)
        params = {"dates": d.strftime("%Y%m%d"), "limit": 50}
        js, status = http_get_json(base, params=params)
        statuses.append(f"{d.isoformat()}: {status}")
        ...
        for ev in js.get("events", []) or []:
            ...
            for c in competitors:
                ...
                if c.get("homeAway") == "home":
                    home = name
                    home_score = safe_float(c.get("score"))
                elif c.get("homeAway") == "away":
                    away = name
                    away_score = safe_float(c.get("score"))
            if home or away:
                rows.append({
                    "game_id": ev.get("id"),
    ...
    if df.empty:
        return df, "No WNBA schedule fallback games found. " + " | ".join(statuses[:3])
    df = df.drop_duplicates(subset=["game_id"], keep="first").sort_values("commence_time")
    return df.reset_index(drop=True), "OK — schedule fallback loaded without Odds API key."


def build_schedule_only_games(schedule_df: pd.DataFrame) -> pd.DataFrame:
    """Convert schedule rows into a safe game table when no betting odds are available."""
    if schedule_df.empty:
        return pd.DataFrame()
    out = schedule_df.copy()
    out["ml_pick"] = "WAIT — add odds key"
    out["ml_prob"] = None
    out["ml_signal"] = "NO ODDS"
    out["spread_pick"] = "WAIT — add odds key"
    out["spread_signal"] = "NO ODDS"
    out["total_pick"] = "WAIT — add odds key"
    out["total_signal"] = "NO ODDS"
    out["total"] = None
    return out
...
def build_game_consensus(odds_df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["ml_signal"] = np.where(out["ml_prob"] >= 0.62, "😈 STRONG ML", np.where(out["ml_prob"] >= 0.58, "✅ LEAN ML", "PASS"))
...
schedule_df, schedule_status = fetch_wnba_schedule_fallback()
games_df = build_game_consensus(odds_df)
if games_df.empty and not schedule_df.empty:
    games_df = build_schedule_only_games(schedule_df)
...
c4.metric("Status", "LIVE" if (not odds_df.empty or not props_df.empty) else ("SCHEDULE" if not games_df.empty else "DIAG"))
...
with tab_games:
    ...
    if games_df.empty:
        ...
        if odds_df.empty:
            st.warning("No Odds API key/live odds loaded, so this is schedule-only. ML/spread/totals need ODDS_API_KEY.")
...
with tab_diag:
    ...
    st.write("Schedule fallback status:", schedule_status)
